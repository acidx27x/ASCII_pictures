...
void setConsoleFont(short width, short height); //set new console font
...
private:
    ...
    unsigned short MAX_WIDTH = 470;

    sf::Image image;

    bool check_file(const std::string& file_name); //check format of picture
    void open_file(); //open file with picture
    void set_picture_size(); //set picture size to console max width (you can change MAX_WIDTH and K_RATIO if your picture is too big or small
    void to_gray_scale(); //set every pixel of resized picture to gray scale gradient
    std::vector<std::vector<char>> convert_to_ascii(); //picture to ascii characters to print in console
    std::vector<std::vector<char>> convert_to_ascii_to_file(); //picture to ascii characters to save in file 
    void print_ascii_picture(const std::vector<std::vector<char>>& converted) const; //printing picture in console
    bool out_file(const std::vector<std::vector<char>>& converted); //out ascii picture in file

    float map_value(const float& value_to_map, const float& start1, const float& stop1, const float& start2, const float& stop2); 
    //finding suetable character from least to brighest
public:
...
void ASCII_picture::to_gray_scale()
{
    for (unsigned short y = 0; y < height; ++y) 
    {
        for (unsigned short x = 0; x < width; ++x) 
